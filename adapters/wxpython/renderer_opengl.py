...
class _GeoBuilder:
    ...
    def disc_fan_tex(self, cx, cy, cz, radius, segments, uv_center=(0.5,0.5), uv_radius=0.5, normal_up=True):
        TWO_PI=math.pi*2.0; uc,vc=uv_center
        for i in range(segments):
            a0=(i/segments)*TWO_PI; a1=((i+1)/segments)*TWO_PI
            x0=cx+math.cos(a0)*radius; z0=cz+math.sin(a0)*radius
            x1=cx+math.cos(a1)*radius; z1=cz+math.sin(a1)*radius
    ...
    def ring_col(self, cx, cy, cz, outer_r, inner_r, thickness, segments, color):
        ...
        for i in range(segments):
            ...
            self.quad_col(
                (cx+outer_r*c0,cy-thickness,cz+outer_r*s0),(cx+outer_r*c1,cy-thickness,cz+outer_r*s1),
                (cx+outer_r*c1,cy+thickness,cz+outer_r*s1),(cx+outer_r*c0,cy+thickness,cz+outer_r*s0),color)
            self.quad_col(
                (cx+inner_r*c1,cy-thickness,cz+inner_r*s1),(cx+inner_r*c0,cy-thickness,cz+inner_r*s0),
                (cx+inner_r*c0,cy+thickness,cz+inner_r*s0),(cx+inner_r*c1,cy+thickness,cz+inner_r*s1),color)
            self.quad_col(
                (cx+inner_r*c0,cy+thickness,cz+inner_r*s0),(cx+inner_r*c1,cy+thickness,cz+inner_r*s1),
                (cx+outer_r*c1,cy+thickness,cz+outer_r*s1),(cx+outer_r*c0,cy+thickness,cz+outer_r*s0),color)
            self.quad_col(
                (cx+outer_r*c0,cy-thickness,cz+outer_r*s0),(cx+outer_r*c1,cy-thickness,cz+outer_r*s1),
    ...
    def ghost_col(self, cx, cy, cz, scale, color, anim_t, segments: int=26):
        r=0.20*scale; seg=segments
        body_layers=11; tail_layers=8; TWO_PI=math.pi*2.0

        def y_and_r(t):
            if t<0.5: return r*0.62*math.cos(t*math.pi),r*0.95*math.sin(t*math.pi)
        ...
        for layer in range(1,body_layers):
            ...
            for i in range(seg):
                ...
                self.quad_col(
                    (cx+ca0*r_prev,cy+y_prev,cz+sa0*r_prev),(cx+ca1*r_prev,cy+y_prev,cz+sa1*r_prev),
        ...
        for layer in range(tail_layers):
            ...
            if layer==0: y_prev_abs=cy-r*0.25; pr_prev=r*0.95
            else: y_prev_abs=cy-r*0.52-pr*r*0.48; pr_prev=r*0.95*(1.0-pr*0.35)
            prev_amp=r*(0.08+0.14*pr)
            for i in range(seg):
                a=(i/seg)*TWO_PI; a1=((i+1)/seg)*TWO_PI
                sk_c0=(math.sin(a*3.0+anim_t*2.4+layer*0.55)*wave_amp+math.sin(a*7.0-anim_t*1.7+layer*0.35)*(wave_amp*0.55))
                sk_c1=(math.sin(a1*3.0+anim_t*2.4+layer*0.55)*wave_amp+math.sin(a1*7.0-anim_t*1.7+layer*0.35)*(wave_amp*0.55))
                if layer==0: sk_p0=sk_p1=0.0
                else:
                    sk_p0=(math.sin(a*3.0+anim_t*2.4+(layer-1)*0.55)*prev_amp+math.sin(a*7.0-anim_t*1.7+(layer-1)*0.35)*(prev_amp*0.55))
                    sk_p1=(math.sin(a1*3.0+anim_t*2.4+(layer-1)*0.55)*prev_amp+math.sin(a1*7.0-anim_t*1.7+(layer-1)*0.35)*(prev_amp*0.55))
                ...
                self.quad_col(
                    (cx+ca0*rp0,y_prev_abs,cz+sa0*rp0),(cx+ca1*rp1,y_prev_abs,cz+sa1*rp1),
    ...
    def gate_bars_col(self, gx, gy_center, gz, wall_h, y_offset, is_jail):
        ...
        if is_jail: self.cube_col(gx,bar_yc+bar_h*0.42,gz,0.47,0.06,0.08,cb)
        else:       self.cube_col(gx,bar_yc+bar_h*0.42,gz,0.08,0.06,0.47,cb)
    ...
    def gate_bars_tex(self, gx, gy_center, gz, wall_h, y_offset, is_jail):
        ...
        cb=(0.65,0.65,0.70,1.0)
        if is_jail: self.cube_tex(gx,bar_yc+bar_h*0.42,gz,0.47,0.06,0.08,uv_scale=1.0)
        else:       self.cube_tex(gx,bar_yc+bar_h*0.42,gz,0.08,0.06,0.47,uv_scale=1.0)
    ...
    def arrow3d_col(self, cx, cy, cz, *, col, shaft_r=0.12, shaft_h=0.60, head_r=0.30, head_h=0.42, seg=32):
        r,g,b,a=col; shaft_top=shaft_h*0.5; shaft_bot=-shaft_h*0.5; tip_y=shaft_bot-head_h
        TWO_PI=math.pi*2.0
        for i in range(seg):
            a0=(i/seg)*TWO_PI; a1=((i+1)/seg)*TWO_PI
            ca0,sa0=math.cos(a0),math.sin(a0); ca1,sa1=math.cos(a1),math.sin(a1)
            self.quad_col(
                (cx+ca0*shaft_r,cy+shaft_top,cz+sa0*shaft_r),(cx+ca1*shaft_r,cy+shaft_top,cz+sa1*shaft_r),
                (cx+ca1*shaft_r,cy+shaft_bot,cz+sa1*shaft_r),(cx+ca0*shaft_r,cy+shaft_bot,cz+sa0*shaft_r),(r,g,b,a))
        apex=(cx,cy+tip_y,cz)
        for i in range(seg):
            a0=(i/seg)*TWO_PI; a1=((i+1)/seg)*TWO_PI
            ca0,sa0=math.cos(a0),math.sin(a0); ca1,sa1=math.cos(a1),math.sin(a1)
            self.tri_col((cx+ca0*head_r,cy+shaft_bot,cz+sa0*head_r),(cx+ca1*head_r,cy+shaft_bot,cz+sa1*head_r),apex,(r,g,b,a))
...
class OpenGLRenderer:
    ...
    def _load_texture(self, path: str) -> Optional[int]:
        ...
        alpha = bytes(img.GetAlpha()) if img.GetAlpha() is not None else bytes([255]) * (w * h)
    ...
    def get_text_texture(
        ...
        *,
        ...
        font = wx.Font(font_size, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, weight, faceName=font_family)
        ...
        rgb = bytes(img.GetData())
        cr, cg, cb_, ca = int(color[0]), int(color[1]), int(color[2]), int(color[3])
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE)
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE)
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, bytes(rgba))
        glBindTexture(GL_TEXTURE_2D, 0)
    ...
    def _get_jail_map_texture(self) -> int:
        ...
        font_big = wx.Font(52, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD)
        ...
        if exit_cells and len(exit_cells) > 0:
            ...
            ex = ox + (ec + 0.5) * cell
            ey = oy + (er + 0.5) * cell
            font_small = wx.Font(22, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD)
    ...
    def _build_lamp_vao(self) -> None:
        ...
        for start_cell in self.core.start_cells:
            if gate_cells:
                ng = min(gate_cells, key=lambda g: abs(g[0]-start_cell[0])+abs(g[1]-start_cell[1]))
        ...
        for exit_cell in self.core.exit_cells:
            if gate_cells:
                ng = min(gate_cells, key=lambda g: abs(g[0]-exit_cell[0])+abs(g[1]-exit_cell[1]))
    ...
    def _draw_dynamic_col_with_flicker(self, raw: _array, mvp: List[float]) -> None:
        ...
        glUniform4f(glGetUniformLocation(self._col_prog, b"uFogColor"), *_SKY_COLOR)
        player = self.core.player
    ...
    def _draw_entities(self, vp: List[float]) -> None:
        ...
        for coin in self.core.coins.values():
            ...
            tmp=_GeoBuilder(textured=False); tmp.coin_col(0.0,0.0,0.0)
            ax=math.pi*0.5
    ...
    def cleanup(self) -> None:
        ...
        for tex in (self._tex_wall, self._tex_floor, self._tex_coin, self._jail_map_texture):
            if tex:
                try: glDeleteTextures(1, [int(tex)])
                except Exception: pass
