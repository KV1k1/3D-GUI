...
class PySide6GameWindow(QMainWindow):
    def __init__(self):
        ...
        self._audio_footsteps = QMediaPlayer(self)
        self._audio_footsteps_out = QAudioOutput(self)
        self._audio_footsteps.setAudioOutput(self._audio_footsteps_out)
        self._audio_footsteps.setSource(QUrl.fromLocalFile(
            os.path.join(self._asset_dir, 'running-on-concrete-268478.mp3')))
        self._audio_footsteps.setLoops(QMediaPlayer.Loops.Infinite)
        self._audio_footsteps_out.setVolume(0.22)
        self._footsteps_playing = False

        self._audio_coin = QMediaPlayer(self)
        self._audio_coin_out = QAudioOutput(self)
        self._audio_coin.setAudioOutput(self._audio_coin_out)
        self._audio_coin.setSource(QUrl.fromLocalFile(
            os.path.join(self._asset_dir, 'drop-coin-384921.mp3')))
        self._audio_coin_out.setVolume(0.55)

        self._audio_gate = QMediaPlayer(self)
        self._audio_gate_out = QAudioOutput(self)
        self._audio_gate.setAudioOutput(self._audio_gate_out)
        self._audio_gate.setSource(QUrl.fromLocalFile(
            os.path.join(self._asset_dir, 'closing-metal-door-44280.mp3')))
        self._audio_gate_out.setVolume(0.55)

        self._door_sound_played = False

        self._audio_ghost = QMediaPlayer(self)
        self._audio_ghost_out = QAudioOutput(self)
        self._audio_ghost.setAudioOutput(self._audio_ghost_out)
        self._audio_ghost.setSource(QUrl.fromLocalFile(
            os.path.join(self._asset_dir, 'ghost-horror-sound-382709.mp3')))
        self._audio_ghost_out.setVolume(0.40)
    ...
    def _start_level(self, level_id: str, *, load_save: bool) -> None:
        ...
        if (not load_save) and float(getattr(self.core, 'elapsed_s', 0.0) or 0.0) <= 0.0001:
            try:
                self._audio_gate.stop()
                self._audio_gate.setPosition(0)
    ...
    def _set_footsteps_playing(self, playing: bool) -> None:
        ...
        if playing:
            self._audio_footsteps.play()
        else:
            self._audio_footsteps.pause()

    def _play_oneshot(self, player: QMediaPlayer) -> None:
        player.stop()
        player.setPosition(0)
        player.play()

    def _on_coin_picked(self, data: dict) -> None:
        self._play_oneshot(self._audio_coin)
        try:
    ...
    def _on_gate_moved(self, data: dict) -> None:
        self._play_oneshot(self._audio_gate)
    ...
    def _play_ghost_sound(self) -> None:
        ...
        self._audio_ghost_out.setVolume(0.06 + 0.55 * t)
        self._play_oneshot(self._audio_ghost)
    ...
    def _on_key_fragment_encountered(self, data: dict) -> None:
        ...
        self._set_footsteps_playing(False)
        self._release_mouse_buttons()
        dlg = Assembly3DMinigame(self, kind=frag_kind or 'KP')
        if dlg.exec():
    ...
    def _toggle_pause(self) -> None:
        ...
        if now_paused:
            ...
            try:
                ...
                pass
            try:
                self._audio_ghost.pause()
            except Exception:
                pass
            try:
                self._audio_coin.pause()
            except Exception:
                pass
            try:
                self._audio_gate.pause()
            except Exception:
                pass
            try:
